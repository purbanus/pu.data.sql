...
public static List<String> converteerTekst( String s, int aRegelLengte )
{
	...
	for ( Iterator it = explode2ToList( ss, ENTER ).iterator(); it.hasNext(); )
	{
		res.addAll( verdeelInRegelsMetVasteKantlijn( (String) it.next(), aRegelLengte ) ); 
	}
	return res;
}
/**
...
public static List<String> verdeelInRegelsMetVasteKantlijn( String s, int regelLengte )
{
	...
	// @@EFFE WEG Assert.assertTrue( chars.length - oudePos <= regelLengte, "Laatste regel groter dan regellengte" );
